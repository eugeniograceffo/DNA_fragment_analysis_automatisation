...
## Peaks calculations (+- 7bp)
Calculate the Area Under the Curve (AUC) as the sum of peaks +- 7bp
met -> 128 bp
unmet -> 386 bp
```{r peak-assign}
## Assign peak type
...
p1 <- ggbarplot(df_plot_tumor, "Sample_name", "AUC_sum",
       add = c("mean_se"),fill = "Peak_type", color = "black", palette = "Paired", facet.by = "Peak_type") +
    ylab("Area Under Curve (AUC)")+
   ggtitle(paste0(x, " condition")) +
  stat_compare_means(label = "p.signif", ref.group = as.character(reference_sample))
...
print(p3)
ggsave(paste0(x, "barplot_Height_max_statistics.png"), plot = p3, path = "./Figures/Statistics")
